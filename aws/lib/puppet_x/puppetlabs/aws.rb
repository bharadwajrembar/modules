...
This could be because some other process is modifying AWS at the same time."""
    ...
    class Aws < Puppet::Provider
      ...
      def iam_client()
        self.class.iam_client()
      end

      def self.elasticache_client(region = default_region)
        ::Aws::ElastiCache::Client.new(client_config(region))
      end

      def elasticache_client(region = default_region)
        self.class.elasticache_client(region)
      end
